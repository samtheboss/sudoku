...
public class UserInterfaceImpl implements IUUserInterfaceContract.View, EventHandler<KeyEvent> {
  private final Stage stage;
  private final Group root;

  private final HashMap<Coordinates, SudokuTextField> textFieldCoordinates;
  private IUUserInterfaceContract.EventListener listener;
  private static final double WINDOW_Y = 732;
  private static final double WINDOW_x = 668;
  private static final double BOARD_PADDING = 732;
  ...
  private void drawGridLines(Group root) {
    ...
    while (index < 8) {
      ...
      Rectangle horizontalLine =
          getLine(BOARD_PADDING, xAndY + 64 * index, thickness, BOARD_X_AND_Y);
  ...
  private void drawTextField(Group root) {
    ...
    final int xAndDelta = 64;
    for (int xIndex = 0; xIndex < 9; xIndex++) {
      for (int yIndex = 0; yIndex < 9; yIndex++) {
        int x = xOrigin + xIndex * xAndDelta;
        int y = yOrigin + yIndex * xAndDelta;
  ...
  @Override
  public void setListener(IUUserInterfaceContract.EventListener listener) {
  ...
  public void handle(KeyEvent keyEvent) {
    if (keyEvent.getEventType() == KeyEvent.KEY_PRESSED) {
      if (keyEvent.getText().matches("0-9")) {
        ...
        handleInputs(value, keyEvent.getSource());
      } else if (keyEvent.getCode() == KeyCode.BACK_SPACE) {
        handleInputs(0, keyEvent.getSource());
      } else {
  ...
  private void handleInputs(int value, Object source) {
    listener.onSudokuInput(
        ((SudokuTextField) source).getX(), ((SudokuTextField) source).getX(), value);
  }
}
